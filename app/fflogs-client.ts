...
export async function completeLogLogin(clientId:string) {
  ...
  requireClientId(clientId);
  if(error)throw new Error("接続できませんでした。");
  ...
  if(!response.ok||typeof payload.access_token!=="string")throw new Error("接続できませんでした。");
...
export function parseLogReportUrl(value:string) {
  ...
  if(!/(^|\.)fflogs\.com$/i.test(url.hostname))throw new Error("対応していない入力です。");
  const reportCode=url.pathname.match(/^\/reports\/([^/]+)/i)?.[1];
  if(!reportCode||!/^[A-Za-z0-9]+$/.test(reportCode))throw new Error("入力を確認できませんでした。");
...
export async function fetchLogReportContext(parsed:{reportCode:string;fightId:number|"last";sourceId:number|null}):Promise<LogReportContext> {
  ...
  if(!report)throw new Error("対象が見つかりませんでした。");
...
export async function fetchAllLogCastEvents({reportCode,fightId,sourceId}:{reportCode:string;fightId:number;sourceId:number}) {
  ...
  if(!page)throw new Error("読み込めませんでした。");
...
async function query(queryText:string,variables:Record<string,unknown>) {
  ...
  if(!response.ok)throw new Error("読み込めませんでした。");
  if(Array.isArray(payload.errors)&&payload.errors.length)throw new Error("読み込めませんでした。");
  return payload.data;
}
...
function requireClientId(clientId:string){if(!clientId)throw new Error("接続設定がありません。");}
