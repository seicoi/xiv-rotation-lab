...
function normalise(row:any):Action{const f=row.fields||{},category=Number(f.ActionCategory?.value),lane=category===2||category===3?"gcd":category===4?"ability":category===9?"limitbreak":"unknown";return{id:Number(row.row_id),name:f.Name||"",lane,level:Number(f.ClassJobLevel)||1,recast:Number(f.Recast100ms||0)/10,cast:Number(f.Cast100ms||0)/10,potency:0,dotPotency:0,dotDuration:0,guaranteedCrit:false,guaranteedDh:false,iconPath:f.Icon?.path||"",hasDamage:Boolean(f.CanTargetHostile)&&Number(f.AttackType?.value)>0,isReplacement:false,isAssignable:f.IsPlayerAction!==false}}
