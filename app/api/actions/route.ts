...
type Action={id:number;name:string;lane:"gcd"|"ability"|"limitbreak"|"unknown";level:number|null;recast:number;cast:number;iconPath:string;hasDamage:boolean;isReplacement:boolean;isAssignable:boolean};

export async function GET(request:Request){
  const url=new URL(request.url),job=(url.searchParams.get("job")||"").toUpperCase(),level=Math.min(100,Math.max(1,Number(url.searchParams.get("level"))||100));
  ...
  try{
    ...
    const initial=await searchAll("Action",query,FIELDS,"ja");
    ...
    const indirections=await searchAll("ActionIndirection",`+ClassJob=${JOB_ROWS[job]}`,"Name,ClassJob,PreviousComboAction","ja");
    ...
    const metadata=await fetchActions(relatedIds);
    ...
    const descriptions=await fetchDescriptions([...unique.values()].map(a=>a.id));
    for(const a of unique.values()){const description=descriptions.get(a.id);if(description)a.hasDamage=/(?:威力\s*[:：]\s*\d|(?<!Cure )Potency\s*:\s*\d|Attacke-Wert\s*:\s*\d|Puissance(?!\s+curative)\s*:\s*\d)/i.test(description)}
    for(const [id,a] of await fetchActions(limitBreakIds(job))){if(a.name)unique.set(`lb:${id}`,{...a,level:null})}
    const actions=[...unique.values()].sort((a,b)=>priority(a)-priority(b)||(a.level??999)-(b.level??999)||a.id-b.id||a.name.localeCompare(b.name,"ja"));
    return Response.json({job,level,count:actions.length,actions},{headers:{"Cache-Control":"public, max-age=86400"}});
...
async function fetchActions(ids:number[]){const result=new Map<number,Action>();for(let i=0;i<ids.length;i+=100){const batch=ids.slice(i,i+100);if(!batch.length)continue;const response=await fetch(`${API}/sheet/Action?${new URLSearchParams({rows:batch.join(","),fields:FIELDS,language:"ja"})}`);if(!response.ok)continue;const data:any=await response.json();for(const row of data.rows||[])result.set(Number(row.row_id),normalise(row))}return result}
async function fetchDescriptions(ids:number[]){const result=new Map<number,string>();for(let i=0;i<ids.length;i+=100){const batch=ids.slice(i,i+100),response=await fetch(`${API}/sheet/ActionTransient?${new URLSearchParams({rows:batch.join(","),fields:"Description",language:"ja"})}`);if(!response.ok)continue;const data:any=await response.json();for(const row of data.rows||[])if(row.fields?.Description)result.set(Number(row.row_id),String(row.fields.Description))}return result}
function normalise(row:any):Action{const f=row.fields||{},category=Number(f.ActionCategory?.value),lane=category===2||category===3?"gcd":category===4?"ability":category===9?"limitbreak":"unknown";return{id:Number(row.row_id),name:f.Name||"",lane,level:Number(f.ClassJobLevel)||1,recast:Number(f.Recast100ms||0)/10,cast:Number(f.Cast100ms||0)/10,iconPath:f.Icon?.path||"",hasDamage:Boolean(f.CanTargetHostile)&&Number(f.AttackType?.value)>0,isReplacement:false,isAssignable:f.IsPlayerAction!==false}}
