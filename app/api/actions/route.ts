...
const FIELDS="Name,ActionCategory.Name,ActionCombo,PreservesCombo,Aspect,Cast100ms,Recast100ms,CooldownGroup,AdditionalCooldownGroup,Icon,IsPlayerAction,CanTargetHostile,AttackType.Name,ClassJobLevel";
const API="https://v2.xivapi.com/api";

type Action={id:number;name:string;lane:"gcd"|"ability"|"limitbreak"|"unknown";level:number|null;recast:number;gcdRecast:number;cast:number;potency:number;comboPotency:number;comboFromActionId:number;preservesCombo:boolean;aspectId:number;attackTypeId:number;attackType:string;dotPotency:number;dotDuration:number;guaranteedCrit:boolean;guaranteedDh:boolean;iconPath:string;hasDamage:boolean;isReplacement:boolean;isAssignable:boolean};
...
function normalise(row:ApiRow):Action{const f=row.fields||{},category=Number(record(f.ActionCategory).value),lane=category===2||category===3?"gcd":category===4?"ability":category===9?"limitbreak":"unknown",recast=Number(f.Recast100ms||0)/10,cooldownGroup=Number(f.CooldownGroup||0),additionalCooldownGroup=Number(f.AdditionalCooldownGroup||0),gcdRecast=lane==="gcd"?(cooldownGroup===58?recast:additionalCooldownGroup===58?0:recast):0,attackType=record(f.AttackType),attackTypeId=Number(attackType.value)||0;return{id:Number(row.row_id),name:String(f.Name||""),lane,level:Number(f.ClassJobLevel)||1,recast,gcdRecast,cast:Number(f.Cast100ms||0)/10,potency:0,comboPotency:0,comboFromActionId:refId(f.ActionCombo),preservesCombo:Boolean(f.PreservesCombo),aspectId:Number(f.Aspect)||0,attackTypeId,attackType:String(record(attackType.fields).Name||""),dotPotency:0,dotDuration:0,guaranteedCrit:false,guaranteedDh:false,iconPath:String(record(f.Icon).path||""),hasDamage:Boolean(f.CanTargetHostile),isReplacement:false,isAssignable:f.IsPlayerAction!==false}}
