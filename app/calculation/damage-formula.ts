...
function rates(stats:FormulaStats,bonuses:RollRateBonuses={}){const mod=LEVEL_MODS[stats.level]||LEVEL_MODS[100],delta=Math.floor(200*(stats.crit-mod.sub)/mod.div);return{critRate:clamp((delta+50)/1000+(bonuses.critRate||0),0,1),critPower:1400+delta,dhRate:clamp(Math.floor(550*(stats.dh-mod.sub)/mod.div)/1000+(bonuses.dhRate||0),0,1)}}
...
const guaranteedRateMultipliers=(guaranteedCrit:boolean,guaranteedDh:boolean,bonuses:RollRateBonuses)=>[guaranteedCrit&&bonuses.critRate?1+bonuses.critRate:1,guaranteedDh&&bonuses.dhRate?1+bonuses.dhRate:1].filter(value=>value!==1);
export function expectedRoll(base:number,stats:FormulaStats,guaranteedCrit=false,guaranteedDh=false,multipliers:number[]=[],bonuses:RollRateBonuses={}){const r=rates(stats,bonuses),cr=guaranteedCrit?1:r.critRate,dh=guaranteedDh?1:r.dhRate,allMultipliers=[...multipliers,...guaranteedRateMultipliers(guaranteedCrit,guaranteedDh,bonuses)],normal=applyMultipliers(base,allMultipliers),crit=applyMultipliers(Math.floor(base*r.critPower/1000),allMultipliers),direct=applyMultipliers(Math.floor(base*125/100),allMultipliers),both=applyMultipliers(Math.floor(Math.floor(base*r.critPower/1000)*125/100),allMultipliers);return normal*(1-cr)*(1-dh)+crit*cr*(1-dh)+direct*(1-cr)*dh+both*cr*dh}
export function simulatedRoll(base:number,stats:FormulaStats,random:()=>number,guaranteedCrit=false,guaranteedDh=false,multipliers:number[]=[],bonuses:RollRateBonuses={}){const allMultipliers=[...multipliers,...guaranteedRateMultipliers(guaranteedCrit,guaranteedDh,bonuses)];if(base<=1)return applyMultipliers(1,allMultipliers);const r=rates(stats,bonuses);let value=base;if(guaranteedCrit||random()<r.critRate)value=Math.floor(value*r.critPower/1000);if(guaranteedDh||random()<r.dhRate)value=Math.floor(value*125/100);value=Math.floor(value*(950+Math.floor(random()*101))/1000);return applyMultipliers(value,allMultipliers)}
export function simulatedDotRoll(base:number,stats:FormulaStats,random:()=>number,guaranteedCrit=false,guaranteedDh=false,multipliers:number[]=[],bonuses:RollRateBonuses={}){const r=rates(stats,bonuses),allMultipliers=[...multipliers,...guaranteedRateMultipliers(guaranteedCrit,guaranteedDh,bonuses)];let value=Math.floor(base*(950+Math.floor(random()*101))/1000);if(guaranteedCrit||random()<r.critRate)value=Math.floor(value*r.critPower/1000);if(guaranteedDh||random()<r.dhRate)value=Math.floor(value*125/100);return applyMultipliers(value,allMultipliers)}
